...
MyPath=r'C:\Users\Administrator\Desktop\毕设文献\数据'#数据文件存储路径

#训练数据
GRNN_Read_Train_Data=pd.read_excel(r'%s\data_set-7-.xlsx'%MyPath)
GRNN_Train_Data=GRNN_Read_Train_Data.values
GRNN_Train_Feature=np.array(GRNN_Train_Data[...,:-1],dtype=float)#训练特征数据[[1 2 3][4 5 6]...]
GRNN_Train_Target=np.array(GRNN_Train_Data[...,-1:],dtype=float)#训练数据[1 -1...]

#测试数据
GRNN_Read_Test_Data=pd.read_excel(r'%s\test_set-7-.xlsx'%MyPath)
GRNN_Test_Data=GRNN_Read_Test_Data.values
GRNN_Test_Feature=np.array(GRNN_Test_Data[...,:-1],dtype=float)#测试特征数据[[1 2 3][4 5 6]...]
GRNN_Test_Target=np.array(GRNN_Test_Data[...,-1:],dtype=float)#测试数据[1 -1...]

#预测数据
GRNN_Read_Predict_Data=pd.read_excel(r'%s\shandong_predict.xlsx'%MyPath)
...
def Grnn_Network(tr_indata,tr_outdata,te_indata,te_outdata,optina_para):
    ...
    try:
        ...
        Error=(((Output_num.T)-te_outdata)**2).sum()/len(te_outdata)
...
def Ou_Grnn(tr_indata,tr_outdata,te_indata,te_outdata,max_num):
    ...
    test_target=(te_outdata)#第一个数值A
    test_output=(dict_data[minkey])

    ################计算R^2#############
    target_data_avg = float(sum(te_outdata) / len(te_outdata))
    
    # 计算分母
    squdiff_den = 0.0
    for item in te_outdata:
    	squdiff_den += math.pow(float(float(item) - target_data_avg), 2)
    squdiff_den /= len(te_outdata)

    # 计算分子
    squdiff_mol = 0.0
    for item in test_output:
    	squdiff_mol += math.pow(float(float(item) - target_data_avg), 2)
    squdiff_mol /= len(te_outdata)


    #测试数据实际值与输出值对比图
    ax3=fig.add_subplot(3,1,1)
    ax3.scatter(range(len(test_target)),test_target,c='g',alpha=1.0,label='Y_target',marker='o')
    ax3.plot(test_output,c='r',alpha=1.0,label='Y_output')
    
    # 
    ax3.set_title('The Contrast between Real and Output; The R^2 is %.4f'%(squdiff_mol/squdiff_den))
    ...
    ax1.plot(list(dict_num.keys()),list(dict_num.values()),c='red',alpha=1)
    ax1.set_title('ErrorNumber[The most suitable parameter is %.2f]'%(minkey/100))
...
#para = Ou_Grnn(trin_data,trout_data,tein_data,teout_data,1000)
...
print(Pre_Function(GRNN_Train_Feature,GRNN_Train_Target,GRNN_Predict_Feature,GRNN_Predict_Target,para/100))
